...
##################################################################################################################
# If You need make any change in MySQL (database, User and password user ). please make same changes in script (master.sh)
# in php_version you can write (1 for V8.1) or (2 for V7.4) or (3 for V5.6) 
php_version="2"
wp_db="wordpress"
wp_user="wordpressuser"
wp_userPassword="P@ssw0rd123@"
mysql_local_or_remote="2"
remote_server="10.1.2.10"
user_remote_server="mahmoud"
password_remote_server="P@ssw0rd123@"
dir_wp_install="/var/www/wordpress"
website_protocol="http"
configuration_directory="wordpress"
server_name="test.local"
set -- "$php_version" "$wp_db" "$wp_user" "$wp_userPassword" "$mysql_local_or_remote" "$remote_server" "$user_remote_server" "$password_remote_server" "$dir_wp_install" "$website_protocol" "$configuration_directory" "$server_name"
...
#params=( "$php_version" "$wp_db" "$wp_user" "$wp_userPassword" "$mysql_local_or_remote" "$remote_server" "$user_remote_server" "$password_remote_server" "$dir_wp_install" "$website_protocol" "$configuration_directory" "$server_name")
#set -- "${params[@]}"
...
<IfModule mod_ssl.c>" | sudo tee /etc/apache2/sites-available/$virtualhost_ssl.conf
                        ...
                        cd $ssl_location && sudo wget -O crt.pem "$certurl" && sudo wget -O key.pem "$privatekey"
